...
| `atcp_get_audio_buf_size(inst)` | 获取音频缓冲区所需大小（float 个数） |
...
└── simulator/                     # C# 三端仿真器
    ...
    ├── SimCommon/                  # 公共库（P/Invoke 封装、TCP 音频传输）
    ├── SimUpper/                   # 上位机仿真器
...
ATCP 提供基于 C# 的三端仿真系统，通过 TCP 模拟音频信道，无需真实硬件即可验证协议栈功能：

| 组件 | 说明 |
|---|---|
| **SimUpper** | 上位机仿真器 — 主动发起连接，发送/接收数据 |
| **SimMiddleware** | 中继仿真器 — TCP 代理，转发上下位机之间的音频流 |
| **SimLower** | 下位机仿真器 — 被动接受连接，回环通信 |
| **SimCommon** | 公共库 — ATCP 原生 DLL 的 P/Invoke 封装 + TCP 音频传输层 |
...
> 仿真器依赖 ATCP 动态库（`atcp.dll`），需先编译核心库。
